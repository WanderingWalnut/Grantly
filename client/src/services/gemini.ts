...
const buildPrompt = (results: GrantSearchResult[]): string => {
  ...
  return [
    ...
    'Before processing, identify duplicate grants that represent the SAME program. Grants are duplicates if they:',
    '- Have the same or very similar program names (e.g., "Community Facility Enhancement Program Small" and "Community Facility Enhancement Program (CFEP) Small" are the SAME program)',
    '- Reference the same funding program even if titles vary slightly (abbreviations, parentheticals, minor wording differences)',
    '- Come from the same source organization for the same program',
    '',
    'For duplicate grants, keep ONLY ONE entry:',
    '- Prefer the FIRST occurrence in the input list (it is the most accurate from the search query)',
    '- If the first occurrence has less complete information, merge the best details from all duplicates',
    '- Use the most complete/accurate title, link, and description',
    '- DO NOT include multiple entries for the same program in your output',
    '',
    ...
    '- summary: Condensed description in 2-3 sentences about what the program funds (merge best details from duplicates if needed)',
    '- eligibility: Array of 2-4 short eligibility requirement phrases (extract from snippet, combine unique requirements from duplicates)',
